...
export class BunqSession implements INodeType {
	...
	description: INodeTypeDescription = {
		...
		version: 1,
		description:
			'Create and manage Bunq API session (installation, device registration, session creation)',
		defaults: {
			name: 'Bunq Session',
		},
		inputs: ['main'],
		...
		properties: [
			{
				...
				default: false,
				description:
					'Whether to force recreation of the entire session (installation, device, and session)',
			},
		],
	};
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		try {
			...
			const sessionData = await ensureBunqSession.call(this, forceRecreate);
