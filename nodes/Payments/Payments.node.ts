...
export class Payments implements INodeType {
	...
	description: INodeTypeDescription = {
		...
		version: 1,
		description:
			'Retrieve payments from a Bunq Monetary Account with pagination and date filtering',
		defaults: {
			name: 'Get Payments',
		},
		inputs: ['main'],
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			try {
				...
				let nextUrl: string | null =
					`/user/${sessionData.userId}/monetary-account/${monetaryAccountId}/payment?count=${itemsPerPage}`;
